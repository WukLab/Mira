...
int main() {
  ...
  for (int i = 0; i < NUM_PAST; ++ i) {
    ...
    StridedMemRefType<float, 4> pkey(past_shape, true);
    DynamicMemRefType<float> dpkey(pkey);
    read_tensor(kdat.c_str(), dpkey, true);

    std::string vdat = basePath + std::to_string(i) + std::string(".value");
    StridedMemRefType<float, 4> pval(past_shape, true);
    DynamicMemRefType<float> dpval(pval);
    read_tensor(vdat.c_str(), dpval, true);
