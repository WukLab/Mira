...
void calculate_haversine_distance_column()
{
    ...
    rring_outer_loop(rdlat, double, N) {
        ...
        for (;_h_rdlat < _t_rdlat + 8 && _h_rdlat < _tlim_rdlat; _h_rdlat ++) {
            if (_tags_rdlat[_h_rdlat % _nblocks_rdlat] == _h_rdlat / _nblocks_rdlat)
                continue;
            else {
                // TODO: SGE?
                rdma(_lbase_rplon + (_h_rdlat % _nblocks_rdlat) * _bsize_rplon,
                     _bsize_rplon,
                     _rbase_rplon + (_h_rdlat * _bsize_rplon), _h_rdlat, IBV_WR_RDMA_READ);
                rdma(_lbase_rplat + (_h_rdlat % _nblocks_rdlat) * _bsize_rplat,
                     _bsize_rplat,
                     _rbase_rplat + (_h_rdlat * _bsize_rplat), _h_rdlat, IBV_WR_RDMA_READ);
                rdma(_lbase_rdlon + (_h_rdlat % _nblocks_rdlat) * _bsize_rdlon,
                     _bsize_rdlon,
                     _rbase_rdlon + (_h_rdlat * _bsize_rdlon), _h_rdlat, IBV_WR_RDMA_READ);
                rdma(_lbase_rdlat + (_h_rdlat % _nblocks_rdlat) * _bsize_rdlat,
                     _bsize_rdlat,
                     _rbase_rdlat + (_h_rdlat * _bsize_rdlat), _h_rdlat, IBV_WR_RDMA_READ);
            }
        }
