...
const size_t s2_nb = 512 * 1024;
const size_t s2_n_block = 128;

// total 30 GB -> local X * 30
rring_init(rvid,       int,        s2_nb * sizeof(int),        s2_n_block, 0, 8192);
rring_init(rpickdate,  SimpleTime, s2_nb * sizeof(SimpleTime), s2_n_block, 0, 8192 + (1ULL << 30));
rring_init(rdropdate,  SimpleTime, s2_nb * sizeof(SimpleTime), s2_n_block, 0, 8192 + (2ULL << 30));
rring_init(rpsgcnt,    int,        s2_nb * sizeof(int),        s2_n_block, 0, 8192 + (3ULL << 30));
rring_init(rtripdist,  double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (4ULL << 30));
rring_init(rplon,      double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (5ULL << 30));
rring_init(rplat,      double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (6ULL << 30));
rring_init(rrateid,    int,        s2_nb * sizeof(int),        s2_n_block, 0, 8192 + (7ULL << 30));
rring_init(rflag,      char,       s2_nb * sizeof(char),       s2_n_block, 0, 8192 + (8ULL << 30));
rring_init(rdlon,      double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (9ULL << 30));
rring_init(rdlat,      double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (10ULL << 30));
rring_init(rptype,     int,        s2_nb * sizeof(int),        s2_n_block, 0, 8192 + (11ULL << 30));
rring_init(rfare,      double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (12ULL << 30));
rring_init(rextra,     double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (13ULL << 30));
rring_init(rmta,       double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (14ULL << 30));
rring_init(rtip,       double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (15ULL << 30));
rring_init(rtolls,     double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (16ULL << 30));
rring_init(rimpv,      double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (17ULL << 30));
rring_init(rtotal,     double,     s2_nb * sizeof(double),     s2_n_block, 0, 8192 + (18ULL << 30));
rring_init(rids,       size_t,     s2_nb * sizeof(size_t),     s2_n_block, 0, 8192 + (19ULL << 30));
rring_init(rpsgcnt_ofld1, int,     s2_nb * sizeof(int),        s2_n_block, 0, 8192 + (20ULL << 30));
rring_init(rpsgcnt_ofld2, int,     s2_nb * sizeof(int),        s2_n_block, 0, 8192 + (21ULL << 30));


void print_passage_counts_by_vendor_id_1(int vendor_id)
{
    ...
    psg_newvec.head = (int *)(4096ULL + (20ULL << 30));
    psg_newvec.end = psg_newvec.head;
    psg_newvec.tail = psg_newvec.head + N;
    ...
    *gep_vids = *vids;
    ...
    int *gep_fid = (int *) (gep_vids + 1);
    *gep_fid = vendor_id;
    arg_size += sizeof(int);

    rvector<int> *gep_psg = (rvector<int> *) (gep_fid + 1);
    *gep_psg = *psg;
    arg_size += sizeof(rvector<int>);
    ...
    *gep_psg_newvec = psg_newvec;
    ...
    // size_t s = passage_count_vec.size();
    rring_outer_loop(rpsgcnt_ofld1, int, s) {

        rring_prefetch(rpsgcnt_ofld1, 16);
        rring_inner_preloop(rpsgcnt_ofld1, int);
        rring_sync(rpsgcnt_ofld1);
        RRING_DEBUG(rpsgcnt_ofld1);

        rring_inner_loop(rpsgcnt_ofld1, j) {
            int e = _inner_rpsgcnt_ofld1[j];
            // printf("%d\n", e);
...
void print_passage_counts_by_vendor_id_2(int vendor_id)
{
    printf("print_passage_counts_by_vendor_id(vendor_id), vendor_id = %d\n", vendor_id);
    ...
    rvector<int> psg_newvec;
    psg_newvec.head = (int *)(4096ULL + (21ULL << 30));
    psg_newvec.end = psg_newvec.head;
    psg_newvec.tail = psg_newvec.head + N;
    ...
    *gep_psg_newvec = psg_newvec;
    ...
    printf("%p %p %p %p\n", indices->head, vids->head, psg->head, psg_newvec.head);

    size_t s = * (size_t *) call_offloaded_service(2, arg_size, sizeof(size_t));

    // size_t s = passage_count_vec.size();
    rring_outer_loop(rpsgcnt_ofld2, int, s) {

        rring_prefetch(rpsgcnt_ofld2, 16);
        rring_inner_preloop(rpsgcnt_ofld2, int);
        rring_sync(rpsgcnt_ofld2);
        RRING_DEBUG(rpsgcnt_ofld2);

        rring_inner_loop(rpsgcnt_ofld2, j) {
            int e = _inner_rpsgcnt_ofld2[j];
            // printf("%d\n", e);
...
_lbase_rpsgcnt_ofld1 = (uint64_t) rbuf + (20ULL << 30) + (8192ULL);
_lbase_rpsgcnt_ofld2 = (uint64_t) rbuf + (21ULL << 30) + (8192ULL);
...
}
{
    ...
    print_passage_counts_by_vendor_id_1(1);
    times[1] = std::chrono::steady_clock::now();
    print_passage_counts_by_vendor_id_2(2);
